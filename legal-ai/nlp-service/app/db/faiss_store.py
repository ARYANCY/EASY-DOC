...
async def search_similar(query_embedding: List[float], k: int = 5) -> List[Tuple[str, float]]:
...
def _search_sync(query_embedding: List[float], k: int) -> List[Tuple[str, float]]:
    ...
    for score, idx in zip(scores[0], indices[0]):
        if idx in _metadata and idx >= 0:
            results.append((_metadata[idx]["text"], float(score)))
